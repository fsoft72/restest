...
class PostmanExporterItem:
	def __init__ ( self, title, mode, url, data, base_url, headers, auth_name, auth_value ):
		self.mode = mode
		self.name = None
		self.url = {}
		self.data = data
	...
	def _parse_headers ( self ):
		if not len ( self.headers ): return
		print ( "\n\n\n***************** HEADERS **************************\n\n", self.headers )
		res = []
		for k, v in self.headers.items ():
			if k == self.auth_name: v = self.auth_value
...
class PostmanExporter:
	...
	def add ( self, title, mode, url, data, headers, r ):
		it = PostmanExporterItem ( title, mode, url, data, self.base_url, headers, self.auth_name, self.auth_value )
